...
const JobCostLedgerEdit: React.FC<JobCostLedgerProps> = ({
    jobCostLedger,
    onSubmit,
    isSubmitting,
    onCancel,
    ...
    const { register, handleSubmit, watch, control, setValue, 
        reset, formState: { errors }, unregister} = useForm<JobCostLedgerInputs>({
                                    ...
                                    <div className="bg-slate-50 border-2 border-slate-200 rounded-lg p-6 relative">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    className="w-full text-sm px-3 py-2 bg-violet-50 border-2 border-violet-200 rounded-lg font-bold text-violet-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:text-green-900 focus:border-transparent transition-all duration-200"
                                                    {...register(`job_cost_ledger_lines.${index}.boq_line`, {required: "select a BOQ"})}
                                                    onChange={onBoqLineChange}
                                                >
                                                    <option value="">Select BOQ Line...</option>
                                                    {boqLinesOptions}
                                                </select>
                                                {errors.job_cost_ledger_lines?.[index]?.boq_line && <p className="text-red-700 text-sm">{errors.job_cost_ledger_lines?.[index]?.boq_line.message}</p>}
                                            </div>
                ...
                >
                    {isSubmitting ? (
                        <span className="flex items-center gap-2">
                            ...
                            'Updating Job Cost Ledger'
                        </span>
                    ) : (
                        'Update Job Cost Ledger'
                    )}
                </button>
                <button
                    type="button"
...
export default JobCostLedgerEdit;