...
function ProjectsProfileManagement() {
    ...
    const { data: projects = [], isLoading: isLoadingProjects, error: projectError} = useQuery({
        queryKey: ['projects'],
    ...
    const filteredProjects = projects.filter((project: any) => {
        const projectNumber = String(project.project_code)?.toLowerCase() || '';
    ...
    // Pagination calculations for Projects Table
    const totalProjectPages = Math.ceil(sortedProjects.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const paginatedProjectsProfiles = sortedProjects.slice(startIndex, startIndex + itemsPerPage);
    ...
    return (
        ...
        <div className="p-8">
            ...
            {view === 'details' && (
            <ProjectsProfileDetails 
                project={selectedProject}
                isLoading={isLoadingProject}
                onBack={handleBackToProjectsList}
                onEdit={handleEditProjectButton}
            />
            )}
