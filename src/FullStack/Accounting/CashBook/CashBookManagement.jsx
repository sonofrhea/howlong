...
function CashBookManagement() {
    ...
    const { data: cashBooks = [], isLoading: isLoadingCashBooks, error: cashBooksError} = useQuery({
        queryKey: ['cashBooks'],
    ...
    const { data: selectedCashBook, isLoading: isLoadingCashBook } = useQuery({
        queryKey: ['cashBook', selectedCashBookId],
        queryFn: () => fetchCashBookById(selectedCashBookId),
        enabled: !!selectedCashBookId,
    });
    // ------------------------------------------------------------------------------------
                    // MANIPULATIONS
    ...
    const handleAddCashBook = async (cashBookData) => {
    ...
    const handleUpdateCashBook = (cashBookData) => {
        updateCashBookMutation.mutate({
    ...
    const handleDeleteCashBook = async (cashBookId) => {
        if (window.confirm('Are you sure you want to delete this cash book?')) {
        deleteCashBookMutation.mutate(cashBookId);
        }
    };
    // ------------------------------------------------------------------------------------
    ...
    const handleEditCashBook = (cashBookId, cashBookData) => {
        setSelectedCashBookId(cashBookId);
        setView('edit');
    };
    // ------------------------------------------------------------------------------------
    ...
    const filteredCashBooks = cashBooks.filter(cashBook => {
        const cashBookNumber = cashBook.reference_number?.toLowerCase() || '';
        const search = searchTerm.toLowerCase();
        
        return cashBookNumber.includes(search);
    });
    ...
    const sortedCashBooks = React.useMemo(() => {
        if (!sortConfig.key) return filteredCashBooks;

        return [...filteredCashBooks].sort((a, b) => {
    ...
    // Pagination calculations for Cash Book Table
    const totalCashBooks = Math.ceil(sortedCashBooks.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const paginatedCashBooks = sortedCashBooks.slice(startIndex, startIndex + itemsPerPage);
    ...
    if (isLoadingCashBooks) return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
            <p className="mt-4 text-gray-600">Loading cash books...</p>
        </div>
        </div>
    );

    if (cashBooksError) return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md text-center">
            ...
            <p className="text-gray-600">Failed to load cash books. Please try again.</p>
        </div>
        </div>
    );
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Cash Book Management</p>
                        </div>
                    </div>
        ...
        <div className="p-8">
            ...
            <div className="mb-12">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <h1 className="text-4xl font-light text-gray-900 tracking-tight">Cash Book</h1>
                        <p className="text-gray-500 mt-2">Manage and track your cash book entries</p>
                    </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={handleBackToCashBooksList}
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="text-center">
                        ...
                        {new Set(cashBooks.map(c => c.currency?.currency_code)).size}
                        </div>
            ...
            {view === 'list' && (
                ...
                <CashBookTable 
                    cashBooks={paginatedCashBooks}
                    onCashBookClick={handleCashBookClick}
                    onEditCashBook={handleEditCashBook}
                    onDeleteCashBook={handleDeleteCashBook}
                    sortConfig={sortConfig}
                    onSort={handleSort}
                    currentPage={currentPage}
                    totalCashBooks={totalCashBooks}
                    totalItems={sortedCashBooks.length}
            ...
            {view === 'form' && (
                ...
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-8">
                    ...
                    <div>
                        <h2 className="text-2xl font-light text-gray-900">Create Cash Book Entry</h2>
                        <p className="text-gray-500">Add a new cash book entry to your records</p>
                    </div>
                    </div>
                    <CashBookForm 
                    onSubmit={handleAddCashBook} 
                    isSubmitting={createCashBookMutation.isLoading} 
            ...
            {view === 'edit' && selectedCashBook && (
                <CashBookEdit 
                cashBook={selectedCashBook}
                onSubmit={handleUpdateCashBook}
                isSubmitting={updateCashBookMutation.isLoading}
                onBack={handleBackToCashBooksList}
                onCancel={handleBackToCashBooksList}
                />
            )}
