...
import { fetchSupplierInvoices, fetchSupplierInvoiceById, createSupplierInvoice,
    updateSupplierInvoice, deleteSupplierInvoice, fetchSupplierProfiles
 } from "../Engines";
...
function SupplierInvoiceManagement() {
    ...
    const { data: supplierInvoices = [], isLoading: isLoadingSupplierInvoices, error: supplierInvoicesError} = useQuery({
    ...
    const { data: selectedSupplierInvoice, isLoading: isLoadingSupplierInvoice } = useQuery({
        queryKey: ['supplierInvoice', selectedSupplierInvoiceId],
        queryFn: () => fetchSupplierInvoiceById(selectedSupplierInvoiceId!),
        enabled: !!selectedSupplierInvoiceId,
    });
    // ------------------------------------------------------------------------------------
                    // MANIPULATIONS
    ...
    const filteredSupplierInvoices = supplierInvoices.filter((supplierInvoice: any) => {
        const invoiceNumber = String(supplierInvoice.invoice_number)?.toLowerCase() || '';
        const supplierName = supplierInvoice.supplier?.supplier_name?.toLowerCase() || '';
        const agentName = supplierInvoice.agent?.username?.toLowerCase() || '';
        const search = searchTerm.toLowerCase();
        
        return invoiceNumber.includes(search) || agentName.includes(search) || supplierName.includes(search);
    });
    ...
    const sortedSupplierInvoices = React.useMemo(() => {
        if (!sortConfig.key) return filteredSupplierInvoices;

        return [...filteredSupplierInvoices].sort((a, b) => {
    ...
    }, [filteredSupplierInvoices, sortConfig]);
    ...
    // Pagination calculations for Supplier Invoice Table
    const totalSupplierInvoicePages = Math.ceil(sortedSupplierInvoices.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const paginatedSupplierInvoices = sortedSupplierInvoices.slice(startIndex, startIndex + itemsPerPage);
        ...
        <div className="p-8">
            ...
            <div className="mb-12">
                ...
                <div className="flex items-center gap-6 justify-between">
                    <div className="flex items-center gap-4">
                    <div className="text-center">
                        <div className="text-2xl font-light text-gray-900">{supplierInvoices.length}</div>
                        <div className="text-sm text-gray-500">Total Supplier Invoices</div>
                    </div>
            ...
            {view === 'list' && (
                ...
                <SupplierInvoiceTable 
                    supplierInvoices={paginatedSupplierInvoices}
                    onSupplierInvoiceClick={handleSupplierInvoiceClick}
                    onEditSupplierInvoice={handleEditSupplierInvoice}
                    onDeleteSupplierInvoice={handleDeleteSupplierInvoice}
                    sortConfig={sortConfig}
                    onSort={handleSort}
                    currentPage={currentPage}
                    totalPages={totalSupplierInvoicePages}
                    totalItems={sortedSupplierInvoices.length}
            ...
            {view === 'form' && (
                ...
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-8">
                    ...
                    <SupplierInvoiceForm 
                    onSubmit={handleAddSupplierInvoice} 
                    isSubmitting={createSupplierInvoiceMutation.isPending} 
            ...
            {view === 'details' && (
                ...
                isLoading={isLoadingSupplierInvoice}
                onBack={handleBackToSupplierInvoicesList}
                onEdit={handleEditSupplierInvoiceButton}
                />
            )}

            {view === 'edit' && selectedSupplierInvoice && (
                <SupplierInvoiceEdit 
                supplierInvoice={selectedSupplierInvoice}
