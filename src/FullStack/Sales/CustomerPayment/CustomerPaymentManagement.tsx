...
function CustomerPaymentManagement() {
    ...
    const { data: customerPayments = [], isLoading: isLoadingCustomerPayments, error: customerPaymentsError} = useQuery({
    ...
    const { data: selectedCustomerPayment, isLoading: isLoadingCustomerPayment } = useQuery({
        queryKey: ['customerPayment', selectedCustomerPaymentId],
        queryFn: () => fetchCustomerPaymentById(selectedCustomerPaymentId!),
        enabled: !!selectedCustomerPaymentId,
    });
    // ------------------------------------------------------------------------------------
                    // MANIPULATIONS
    ...
    const filteredCustomerPayments = customerPayments.filter((customerPayment: any) => {
        ...
        return paymentNumber.includes(search) || customerName.includes(search);
    });
    ...
    const sortedCustomerPayments = React.useMemo(() => {
    if (!sortConfig.key) return filteredCustomerPayments;

    return [...filteredCustomerPayments].sort((a, b) => {
    ...
    }, [filteredCustomerPayments, sortConfig]);
    ...
    // Pagination calculations for Customer Payment Table
    const totalCustomerPayments = Math.ceil(sortedCustomerPayments.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const paginatedCustomerPayments = sortedCustomerPayments.slice(startIndex, startIndex + itemsPerPage);
    ...
    if (customerPaymentsError) return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        ...
        <p className="text-gray-600">Failed to load customer payment. Please try again.</p>
        </div>
    </div>
    );
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Customer Payment Management</p>
                        </div>
                    </div>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-start justify-between mb-8">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                    <h1 className="text-4xl font-light text-gray-900 tracking-tight">Customer Payments</h1>
                    <p className="text-gray-500 mt-2">Manage and track your customer payment transactions</p>
                    </div>
                </div>
                </div>
                ...
                {(view === 'form' || view === 'details' || view === 'edit') && (
                    <button
                    onClick={handleBackToCustomerPaymentsList}
                    className="bg-white border border-gray-200 hover:border-gray-300 text-gray-700 px-6 py-3 rounded-xl font-medium transition-all duration-200 flex cursor-pointer items-center gap-2 hover:shadow-sm"
                    >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
            ...
            {view === 'list' && (
                ...
                <div className="flex items-center gap-4">
                    <div className="text-center">
                    <div className="text-2xl font-light text-gray-900">{customerPayments.length}</div>
                    <div className="text-sm text-gray-500">Total Customer Payments</div>
                    </div>
                    <div className="w-px h-8 bg-gray-200"></div>
                    <div className="text-center">
                    <div className="text-2xl font-light text-gray-900">
                        {new Set(customerPayments.map((c: any) => c.currency?.currency_code)).size}
                    </div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                ...
                customerPayments={paginatedCustomerPayments}
                onCustomerPaymentClick={handleCustomerPaymentClick}
                onEditCustomerPayment={handleEditCustomerPayment}
                onDeleteCustomerPayment={handleDeleteCustomerPayment}
                sortConfig={sortConfig}
                onSort={handleSort}
                currentPage={currentPage}
                totalPages={totalCustomerPayments}
                totalItems={sortedCustomerPayments.length}
            ...
            <div className="w-full bg-gray-50 rounded-2xl shadow-sm border border-gray-200">
                ...
                <div className="flex items-center gap-4 mb-8 justify-between">
                    ...
                    <div>
                    <h2 className="text-2xl font-light text-gray-900">Create Customer Payment</h2>
                ...
                <CustomerPaymentForm 
                    onSubmit={handleAddCustomerPayment} 
                    isSubmitting={createCustomerPaymentMutation.isPending} 
                ...
                {createCustomerPaymentMutation.isError && (
                    ...
                    Error creating customer payment. Please try again.
                    </div>
                )}
                </div>
            </div>
            ...
            {view === 'edit' && selectedCustomerPayment && (
            <CustomerPaymentEdit 
                customerPayment={selectedCustomerPayment}
